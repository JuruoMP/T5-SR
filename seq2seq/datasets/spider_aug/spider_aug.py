...
class Spider(datasets.GeneratorBasedBuilder):
    ...
    def __init__(self, *args, writer_batch_size=None, **kwargs):
        ...
        self.sql_parser = SqlParser('data/spider/tables.json', db_dir='data/database')
        self.train_data_path = 'data/spider_aug/augment.txt'
        ...
        return [
            datasets.SplitGenerator(
                ...
                gen_kwargs={
                    ...
                    "data_filepath": self.train_data_path,
    ...
    def _generate_examples(self, data_filepath, db_path):
        ...
        with open(data_filepath, encoding="utf-8") as f:
            try:  # raw data
                ...
                for idx, sample in enumerate(spider):
                    ...
                    yield idx, {
                        ...
                        "query": self.sql_parser.dict_to_sql(db_id, sample["sql"]),
            ...
            except:  # augmentation data
                real_idx = 0
                for line in open(data_filepath, encoding="utf-8").readlines():
                    try:
                        idx, db_id, text, sql = line.strip().split('\t')
                        if db_id not in self.schema_cache:
                            self.schema_cache[db_id] = dump_db_json_schema(
                                db_path + "/" + db_id + "/" + db_id + ".sqlite", db_id
                            )
                        ...
                        yield real_idx, {
                            "raw_query": sql,
                            "query": self.sql_parser.dict_to_sql(db_id, self.sql_parser.raw_sql_to_dict(db_id, sql)),
                            "question": text,
                            "db_id": db_id,
                            "db_path": db_path,
                            "db_table_names": schema["table_names_original"],
