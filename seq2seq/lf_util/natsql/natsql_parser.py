...
from seq2seq.lf_util.natsql.natsql2sql import inference_sql,Args,natsql_version as n_version
import sqlite3
import sys,os,copy
import json
from nltk import word_tokenize
from seq2seq.lf_util.natsql.process_sql import get_tables_with_alias, Schema, get_schema
from seq2seq.lf_util.natsql.process_sql import parse_sql as parse_sql_original
from seq2seq.lf_util.natsql.process_sql import tokenize as tokenize_original
from seq2seq.lf_util.natsql.preprocess.sql_back import sql_back
...
class NatsqlParser:
    def __init__(self, table_path, db_dir):
        ...
        self.db_dir = db_dir
        self.table_dict = {x['db_id']: x for x in json.load(open(table_path))}
    ...
    def natsql_to_sql(self, db_id, natsql):
        ret = self.create_sql_from_natSQL(natsql, db_id,
                                          os.path.join(self.db_dir, db_id, f'{db_id}.sqlite'), self.table_dict[db_id])
        return ret[0]

    def raw_sql_to_dict(self, db_id, raw_sql):
        return self.raw_sql_dict_parser.unparse(db_id, raw_sql)
        ...
        if toks[idx] == 'select':
            ...
            try:
                ...
                while end_idx < len_ and toks[end_idx] != ',' and toks[end_idx] != ')' \
                        and toks[end_idx] != 'join' and toks[end_idx] not in CLAUSE_KEYWORDS2 and toks[
    ...
    def parse_sql(self, toks, start_idx, tables_with_alias, schema):
        ...
        # select clause
        _, select_col_units = self.parse_select(toks, start_idx, tables_with_alias, schema, default_tables)
        idx = from_end_idx
        ...
        if idx < len_ and toks[idx] in SQL_OPS:
            sql_op = toks[idx]
            idx += 1
            idx, IUE_sql = self.parse_sql(toks, idx, tables_with_alias, schema)
            sql[sql_op] = IUE_sql
        return idx, sql
    ...
    def create_sql_from_natSQL(self, nsql, db_name, db, table_json, sq=None, remove_values=False,
                               remove_groupby_from_natsql=False, args=Args()):
        ...
        _, p_nsql = self.parse_sql(toks, 0, tables_with_alias, schema)

        if remove_values and sq:
            p_nsql = self.remove_condition_values(p_nsql)
...
if __name__ == '__main__':
    natsql_parser = NatsqlParser('data/spider_natsql/tables_for_natsql.json', 'data/database')
