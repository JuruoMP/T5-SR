...
from seq2seq.lf_util.evaluation import get_evaluator, evaluate_pair, build_foreign_key_map_from_json
...
class SemqlDictParser:
    ...
    def _unparse(self, sql_dict):
        ...
        if sql_dict['where'] or sql_dict['having']:
            # where_clause = self._unparse_where(sql_dict['where'])
            where_clause = self._unparse_where_having(sql_dict['where'], sql_dict['having'])
        if sql_dict['groupBy']:
    ...
    def _unparse_where_having(self, _where, _having):
        where_clause = self._unparse_condition(_where)
        having_clause = self._unparse_condition(_having)
        clauses = []
        if where_clause or having_clause:
            clauses.append('WHERE')
            if where_clause and having_clause:
                clauses += [where_clause, 'AND', having_clause]
            else:
                clauses += [where_clause, having_clause]
        return ' '.join([x for x in clauses if x != ''])
...
class SemqlStrParser:
    ...
    def _parse_sql(self, tokens, start_idx):
        ...
        if 'where' in self.clause_list or 'having' in self.clause_list:
            sql_dict['where'], sql_dict['having'], idx = self._parse_where_having(tokens, idx)
    ...
    def _parse_where_having(self, tokens, start_idx):
        if start_idx >= len(tokens) or tokens[start_idx] != 'where':
            return [], [], start_idx
        idx = start_idx + 1
        conds, idx = self._parse_condition(tokens, idx)
        ...
        for cond in conds:
            if isinstance(cond, str):
                and_or = cond
            elif cond[2][1][0] == 0:  # agg_op != None
                where_conds += [and_or, cond]
                and_or = ''
            else:
                having_conds += [and_or, cond]
                and_or = ''
        where_conds = [x for x in where_conds if x]
        having_conds = [x for x in having_conds if x]
        return where_conds, having_conds, idx
...
class SqlParser:
    def __init__(self, table_path, db_dir='data/database'):
        ...
        self.semql_dict_parser = SemqlDictParser(table_path)
        self.semql_str_parser = SemqlStrParser(table_path)
    ...
    def sql_to_dict(self, db_id, sql):
        return self.semql_str_parser.parse(db_id, sql)

    def dict_to_sql(self, db_id, sql_dict):
        return self.semql_dict_parser.unparse(db_id, sql_dict)
    ...
    def check_equal_script(self, db_name, pred_dict, gold_dict):
        return evaluate_pair(self.script_evaluator, self.kmaps, self.schemas[db_name][1], db_name, pred_dict, gold_dict)
...
if __name__ == '__main__':
    ...
    data = json.load(open('data/spider/train_spider.json'))
    correct, wrong = 0, 0
    error_convert, error_check = 0, 0
    for i in range(len(data)):
        # print(dev_data[i]['question'])
        db_name = data[i]['db_id']
        pred_sql = sql_parser.dict_to_sql(db_name, data[i]['sql'])
        raw_sql = data[i]['query']
        # print(pred_sql)
        # print(raw_sql)
        # print()
        try:
