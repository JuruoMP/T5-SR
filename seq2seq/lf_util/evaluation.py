...
from seq2seq.lf_util.ssql.process_sql import get_schema, Schema, get_sql
...
class Evaluator:
    ...
    def eval_exec_match(self, db_name, pred_sql, gold_sql, pred_dict, gold_dict):
        return eval_exec_match(db_name, pred_sql, gold_sql, pred_dict, gold_dict)
...
def evaluate_pair(evaluator, kmaps, schema, db_name, p_str, g_str):
...
def evaluate_pair_exec(evaluator, kmaps, schema, db_name, p_str, g_str, p_dict, g_dict):
    if not p_str or not g_str:
        return False
    exact_score = evaluator.eval_exec_match(db_name, p_str, g_str, p_dict, g_dict)
    return exact_score
...
if __name__ == '__main__':
    acc = evaluate('data/sparc/dev_gold.txt', '/Users/liyuntao/tmp/predict.txt', 'data/sparc/database', 'data/sparc/tables.json')
