    ...
    def evaluation_loop(
        ...
        for step, inputs in enumerate(dataloader):
            ...
            loss, logits, labels, scores = self.prediction_step(model, inputs, prediction_loss_only, ignore_keys=ignore_keys,
                                                        num_return_sequences=num_return_sequences,output_scores=output_scores,return_dict_in_generate=return_dict_in_generate)

            # Update containers on host
            if loss is not None:
                losses = self._nested_gather(loss.repeat(batch_size*num_return_sequences))
            ...
            if scores is not None:
                scores_gathered = self._nested_gather(scores)
                scores_host = scores_gathered if scores_host is None else nested_concat(scores_host, scores_gathered, padding_index=-100)
            if labels is not None:
        ...
        if not isinstance(eval_dataset, IterableDataset):
            num_samples = len(eval_dataset) * num_return_sequences
